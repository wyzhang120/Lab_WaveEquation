...
s=ricker(mod1.fc,dt);
test =CSG(sx, sz, gx, gz);
test = test.getSeis(vel,nbc,dx,nt,dt,mod1.fc,isFS);
test.plotCSG();
test.plotTrace(idxTrace);
tMute=0.066;
wp0 = test.getWP(idxTrace, tMute);
test.wpHist(wp0);
test.plotWP(wp0, idxTrace, [-1e-6 1e-6]);
